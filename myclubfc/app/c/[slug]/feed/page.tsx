...
export default async function FeedPage({ searchParams }: PageProps) {
  ...
  if (process.env.NEXT_PUBLIC_SUPABASE_URL && club.id !== "kfc-mock-id") {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 md:px-0">
      <PageHeader title="Feed" subtitle={`Tudo que rola no ${club.nome.split(" ")[0]}`} />

      <div className="mb-4 flex gap-2 rounded-lg bg-slate-100 p-1">
        <TabLink href={`${base}/feed?tab=publico`} active={tab === "publico"}>
          <Globe size={14} /> Público
        </TabLink>
        <TabLink href={`${base}/feed?tab=interno`} active={tab === "interno"}>
